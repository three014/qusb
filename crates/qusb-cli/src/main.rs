...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::builder()
                .parse("qusb-cli=debug,qusb=trace")
                .unwrap(),
        )
        .with_line_number(true)
        .with_writer(Mutex::new(BufWriter::with_capacity(BUF_LEN, log_file)))
        .init();
    ...
    match args.command {
        Commands::Serve {
            ...
            let cfg = if make_self_signed {
                make_self_signed_cfg("localhost").0
            } else {
                unimplemented!()
            };

            let server = qusb::server(cfg, bind, num_vhci_ports.0).await?;
            let handle = server.serve();

            _ = compio_signal::ctrl_c()
                .await
...
pub fn make_self_signed_cfg(hostname: &str) -> (rustls::ServerConfig, rcgen::CertifiedKey) {
