...
mod tests {
    ...
    async fn version_sends_properly() {
        ...
        let mut tx = Sender::<proto::Version, Vec<u8>>::new(tx_buf);
        tracing::debug!("About to send my message!");
        tx.send(&proto::VERSION).await.unwrap();

        let rx_buf = tx.tx;
        let mut rx = Receiver::<proto::Version, BufReader<&[u8]>>::new(BufReader::with_capacity(
            256, &*rx_buf,
        ));
        tracing::debug!("About to recv my message!");
        let version = rx.recv().await.unwrap();

        assert_eq!(version.unwrap(), proto::VERSION);
    }
    ...
    async fn can_send_multiple_messages_with_same_type() {
        ...
        let data = proto::UsbDeviceInfo {
            id: proto::UsbDeviceId {
                bus_number: 2,
                device_addr: 1,
            },
            bus_id: proto::BusId(std::borrow::Cow::Borrowed("1-2".try_into().unwrap())),
            vendor_id: 2,
            product_id: 6,
            class: 4,
            subclass: 45,
        ...
        data2.id = proto::UsbDeviceId {
            bus_number: 29,
            device_addr: 45,
        };
        data2.bus_id = proto::BusId(std::borrow::Cow::Borrowed("1-2.4.5".try_into().unwrap()));

        let mut tx = Sender::<proto::UsbDeviceInfo, Vec<u8>>::new(tx_buf);
        tracing::debug!("About to send my messages!");
        tx.send(&data2).await.unwrap();
        tx.send(&data).await.unwrap();

        let rx_buf = tx.tx;
        let mut rx = Receiver::<proto::UsbDeviceInfo, BufReader<&[u8]>>::new(
